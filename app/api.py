...
from iris import main, predict

# HOwever this one is used at the reload and make sure to be same as main.py file
# Define application
app = FastAPI(
    title="health_app",
    description="Predict health conditions",
    version="0.1",
)
...
def load_artifacts():
    ...
    artifacts = main.load_artifacts(run_id=run_id, best_f1=True)
...
def _predict(request: Request, payload: PredictPayload) -> Dict:
    """Predict heal for a list of texts using the best run."""
    # Predict

    texts = [item.text for item in payload.texts]
    # print(texts)
    texts = [[float(i) for i in t.split(",")] for t in texts]
...
def _performance(request: Request, filter: Optional[str] = None) -> Dict:
    ...
    return response
>>>>>>> 12b5da4334fa4fc9fd1d808b15ba507b517d7865
